#Physical Attack: playerUnits[xx].DiminishModifiers([1,3,4,5])
#Magical Attack: playerUnits[xx].DiminishModifiers([2,3,4,5])
#Mental Support: playerUnits[xx].DiminishModifiers([2,3,5])
label FIGHT_05_SAMPLE_UNIT_X:
    # AoE: targetChoice = -1
    python:
        eName = enemyUnits[targetChoice].name
        pName = playerUnits[XXX].name

        HighlightPlayerUnitBars([0,1,2,3])
        xStr = renpy.random.choice(["A","B"])

        xDmg = -(renpy.random.randint(1,3) + playerUnits[0].cPower("brawn"))
        enemyUnits[targetChoice].modifyHP(xDmg,0.0,"guts")

        xHeal = limitValue(playerUnits[0].cPower("occult")+renpy.random.randint(7,10), 7, 20)
    ...
    if isRollSuccess:
        Narrator "Move is performed."
    else:
        Narrator "Move has failed."
...
label FIGHT_05_MW_ROBYN_0:
    ...
    if isRollSuccess:
        ...
        call FIGHT_05_MW_MISTWALKER_HURT(targetChoice) from _call_FIGHT_05_MW_MISTWALKER_HURT
        python:
    ...
    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK
    jump expression currentLabelET

#FOCUS
label FIGHT_05_MW_ROBYN_1:
    $pName = playerUnits[0].name
    ...
    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_1
    jump expression currentLabelET

#SLEIGHT OF CROWBAR
label FIGHT_05_MW_ROBYN_2:
    python:
        eName = enemyUnits[targetChoice].name
        pName = playerUnits[0].name

    call FIGHT_05_MW_MISTWALKER_HURT(targetChoice) from _call_FIGHT_05_MW_MISTWALKER_HURT_1
    if isRollSuccess:
        python:
    ...
    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_2
    jump expression currentLabelET

#FLEX
label FIGHT_05_MW_ROBYN_3:
    $pName = playerUnits[0].name
    ...
    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_3
    jump expression currentLabelET

#CHEER
label FIGHT_05_MW_ROBYN_4:
    ...
    python:
        ...
        #Random Text
        if targetChoice != -1:
            xArray = ["I've got this!", "Oz, you're frightening, but in a cool way!","Go get 'em tiger!", "I um, I feel like a third wheel here!"]
    ...
    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_4
    jump expression currentLabelET

#Healing Spirit
label FIGHT_05_MW_ROBYN_5:
    python:
        pName = playerUnits[0].name
    ...
    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_5
    jump expression currentLabelET

#Kazap
label FIGHT_05_MW_ROBYN_6:
    call FIGHT_05_MW_MISTWALKER_HURT(targetChoice) from _call_FIGHT_05_MW_MISTWALKER_HURT_2
    python:
        eName = enemyUnits[targetChoice].name
        pName = playerUnits[0].name
    ...
    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_6
    jump expression currentLabelET
# -----------------------------------------------------------------------(1) GUS
#Onslaught
label FIGHT_05_MW_GUS_0:
    python:
    ...
    call FIGHT_05_MW_GUS_0_SLASH(-1) from _call_FIGHT_05_MW_GUS_0_SLASH

    if renpy.random.choice(hitAcc) and enemyUnits[targetChoice].isAlive:
        call FIGHT_05_MW_GUS_0_SLASH(1) from _call_FIGHT_05_MW_GUS_0_SLASH_1

    $hitAcc.append(False)
    if renpy.random.choice(hitAcc) and enemyUnits[targetChoice].isAlive:
        call FIGHT_05_MW_GUS_0_SLASH(3) from _call_FIGHT_05_MW_GUS_0_SLASH_2

    call FIGHT_05_MW_GUS_REGEN from _call_FIGHT_05_MW_GUS_REGEN
    $playerUnits[1].DiminishModifiers([1,3,4,5])
    call FIGHT_05_MW_DESPOOK(1) from _call_FIGHT_05_MW_DESPOOK_7
    jump expression currentLabelET

label FIGHT_05_MW_GUS_0_SLASH(xMod=0):
    call FIGHT_05_MW_MISTWALKER_HURT(targetChoice) from _call_FIGHT_05_MW_MISTWALKER_HURT_3
    python:
        #Enemy Damage
...
label FIGHT_05_MW_GUS_1:
    ...
    call FIGHT_05_MW_GUS_REGEN from _call_FIGHT_05_MW_GUS_REGEN_1
    call FIGHT_05_MW_DESPOOK(1) from _call_FIGHT_05_MW_DESPOOK_8
    jump expression currentLabelET

#Bite Back
# Counter
label FIGHT_05_MW_GUS_2:
    python:
        if isRollSuccess:
    ...
    call FIGHT_05_MW_DESPOOK(1) from _call_FIGHT_05_MW_DESPOOK_9
    jump expression currentLabelET

#Sniff (Sniff it Out)
# Lowers Target Difficulty Mod
label FIGHT_05_MW_GUS_3:
    python:
        eName = enemyUnits[targetChoice].name
        pName = playerUnits[1].name
    ...
    $playerUnits[1].DiminishModifiers([2,3,5])
    call FIGHT_05_MW_DESPOOK(1) from _call_FIGHT_05_MW_DESPOOK_10
    jump expression currentLabelET

label FIGHT_05_MW_GUS_REGEN:
    $playerUnits[1].modifyHP(2,1.0,"guts")
    Narrator "Blessed by the moon's silvery light, August recovers 2 [kwHealth]!"
    return

label FIGHT_05_MW_GUS_COUNTER(eDex=0):
...
label FIGHT_05_MW_OZ_0:
    ...
    if (isRollSuccess):
        call FIGHT_05_MW_MISTWALKER_HURT(targetChoice) from _call_FIGHT_05_MW_MISTWALKER_HURT_4
    ...
    call FIGHT_05_MW_DESPOOK(2) from _call_FIGHT_05_MW_DESPOOK_11
    jump expression currentLabelET

#Glare
label FIGHT_05_MW_OZ_1:
    python:
        xVar = dieTotal + playerUnits[2].cStats("brains")
    ...
    $playerUnits[2].DiminishModifiers([2,3,4,5])
    call FIGHT_05_MW_DESPOOK(2) from _call_FIGHT_05_MW_DESPOOK_12
    jump expression currentLabelET

#Gut Punch
label FIGHT_05_MW_OZ_2:
    call FIGHT_05_MW_MISTWALKER_HURT(targetChoice) from _call_FIGHT_05_MW_MISTWALKER_HURT_5
    python:
    ...
    call FIGHT_05_MW_DESPOOK(2) from _call_FIGHT_05_MW_DESPOOK_13
    jump expression currentLabelET

#Bandage Wounds
label FIGHT_05_MW_OZ_3:
    python:
    ...
    call FIGHT_05_MW_DESPOOK(2) from _call_FIGHT_05_MW_DESPOOK_14
    jump expression currentLabelET

# ---------------------------------------------------------------------- ENEMIES
# ---------------------------------------------------------------------- ENEMIES
# ---------------------------------------------------------------------- ENEMIES
label FIGHT_05_MW_MISTWALKER_HURT(eMan=0):
    if eMan == 0:
...
#currentCA
# ----------------------------------------------------------------(0) MISTWALKER
label FIGHT_05_MW_MISTWALKER_ATTACK:
    if currentCA == 1:
        call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_15
        call FIGHT_05_MW_DESPOOK(1) from _call_FIGHT_05_MW_DESPOOK_16
        call FIGHT_05_MW_DESPOOK(2) from _call_FIGHT_05_MW_DESPOOK_17
    elif currentCA == 3:
        call FIGHT_05_MW_MISTWALKER_BUFFLANTERN from _call_FIGHT_05_MW_MISTWALKER_BUFFLANTERN
    ...
    python:
        ...
        HighlightPlayerUnitBars([0,1,2,3])
        if numWisp > 1:
            xAtk = renpy.random.randint(1,3)
        else:
            xAtk = renpy.random.randint(1,2)

        if numWisp > 3:
            xAtk = 3
        elif xAtk != 0 and xAtk != 3:
            eTarget = [playerUnits[0].calcAggro(),playerUnits[1].calcAggro(),playerUnits[2].calcAggro(),0]
            eTarget = AIChooseTarget(eTarget)
            HighlightPlayerUnitBars([eTarget])


    play sfx boss_target_sfx
    call expression "FIGHT_05_MW_MISTWALKER_" + str(xAtk) from _call_expression_34
    call FIGHT_05_MW_GUS_COUNTER(0) from _call_FIGHT_05_MW_GUS_COUNTER
    jump expression currentLabelPT

label FIGHT_05_MW_MISTWALKER_BUFFLANTERN:
    python:
...
label FIGHT_05_MW_MISTWALKER_0:
    #NOTE: THIS IS CURRENTLY BEING UNUSED DW ABOUT THIS MOVE

    Narrator "A bone-chilling wind breezes through the graveyard."

    call dice_roll(avgUnitStat(playerUnits,"brains"), enemyUnits[0].cDifficulty("occult"), "Shrill Wind") from _call_dice_roll_14
        ...
        python:
            for x in range(len(playerUnits)):
                if playerUnits[x].isAlive:
                    playerUnits[x].modifyStatMod("guts",2,-4,4)
    else:
        python:
            xDmg = -(renpy.random.randint(-1,1) + enemyUnits[0].cPower("occult") + int(numWisp*.5))
            for x in range(len(playerUnits)):
                playerUnits[x].modifyHP(xDmg,0.0,"brains")

        Narrator "The party is struck with a devestating gale!"
        python:
...
label FIGHT_05_MW_MISTWALKER_1:
    python:
        eName = enemyUnits[0].name
    ...
    call dice_roll(playerUnits[eTarget].cStats("hustle"), enemyUnits[0].cDifficulty("brawn"), "Mysterious Claw") from _call_dice_roll_15
...
label FIGHT_05_MW_MISTWALKER_2:
    python:
        eName = enemyUnits[0].name
        pName = playerUnits[eTarget].name
    ...
    call dice_roll(playerUnits[eTarget].cStats("guts"), enemyUnits[0].cDifficulty("occult"), "Clinging Fog") from _call_dice_roll_16
...
label FIGHT_05_MW_MISTWALKER_3:
    ...
    call dice_roll(xNum, enemyUnits[0].cDifficulty("occult"), "Lights Out") from _call_dice_roll_17
    ...
    python:
        eName = enemyUnits[0].name
        pName = playerUnits[eTarget].name
        xDmg = -int(enemyUnits[0].cPower("occult") + numWisp*2 - 1)
    ...
    call FIGHT_05_MW_SETWISPS(0) from _call_FIGHT_05_MW_SETWISPS
    with { "master" : Dissolve(0.5) }
    Narrator "The wisps fade."
    return

label FIGHT_05_MW_SETWISPS(xWisps=0):
    if numWisp < xWisps:
...
label FIGHT_05_MW_DESPOOK(pDex=0):
...
# -------------------------------------------------------------------(0) LANTERN
define wowName = "will-o-wisp"
label FIGHT_05_MW_LANTERN_ATTACK:
    ...
    if currentCA == 1 and numWisp < 5:
        call FIGHT_05_MW_LANTERN_0 from _call_FIGHT_05_MW_LANTERN_0
    elif numWisp == 5:
        if not enemyUnits[0].isAlive:
            call FIGHT_05_MW_LANTERN_2 from _call_FIGHT_05_MW_LANTERN_2
        elif enemyUnits[1].isBloodied() and enemyUnits[1].stats["gutsMod"] > -4:
            call FIGHT_05_MW_LANTERN_1 from _call_FIGHT_05_MW_LANTERN_1
        if renpy.random.choice([True,False]):
            call FIGHT_05_MW_LANTERN_3 from _call_FIGHT_05_MW_LANTERN_3
        else:
            call FIGHT_05_MW_LANTERN_2B from _call_FIGHT_05_MW_LANTERN_2B

    else:
        if not enemyUnits[0].isAlive:
            call FIGHT_05_MW_LANTERN_2 from _call_FIGHT_05_MW_LANTERN_2_1
        elif enemyUnits[1].isBloodied() and enemyUnits[1].stats["gutsMod"] > -4:
            call FIGHT_05_MW_LANTERN_1 from _call_FIGHT_05_MW_LANTERN_1_1
        else:
            call FIGHT_05_MW_LANTERN_0 from _call_FIGHT_05_MW_LANTERN_0_1

    jump expression currentLabelPT

#Summon Will-o-wisp (Max: 5)
label FIGHT_05_MW_LANTERN_0:
    python:
        eName = enemyUnits[1].name

    call FIGHT_05_MW_SETWISPS(limitValue(numWisp+1,0,5)) from _call_FIGHT_05_MW_SETWISPS_1
    Narrator "[eName] summons a [wowName]."

    return

#Unstable
label FIGHT_05_MW_LANTERN_1:
    call FIGHT_05_MW_SETWISPS(limitValue(numWisp-1,0,5)) from _call_FIGHT_05_MW_SETWISPS_2
    ...
    python:
        ...
        enemyUnits[0].modifyStatMod("power",1,-4,4)

    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_18
    call FIGHT_05_MW_DESPOOK(1) from _call_FIGHT_05_MW_DESPOOK_19
    call FIGHT_05_MW_DESPOOK(2) from _call_FIGHT_05_MW_DESPOOK_20
    return

#Heal Mistwalker
label FIGHT_05_MW_LANTERN_2:
    ...
    call FIGHT_05_MW_SETWISPS(limitValue(numWisp-3,0,5)) from _call_FIGHT_05_MW_SETWISPS_3
    Narrator "[eName] uses the [wowName]s to heal [e2Name] for [xHeal] [kwHealth]!"

    call FIGHT_05_MW_DESPOOK(0) from _call_FIGHT_05_MW_DESPOOK_21
    call FIGHT_05_MW_DESPOOK(1) from _call_FIGHT_05_MW_DESPOOK_22
    call FIGHT_05_MW_DESPOOK(2) from _call_FIGHT_05_MW_DESPOOK_23
    return

#Heal Mistwalker
label FIGHT_05_MW_LANTERN_2B:
    if numWisp > 0:
        python:
            HighlightEnemyUnitBars([0,1])
            xHeal = (numWisp*2)
...
label FIGHT_05_MW_LANTERN_3:
    python:
        eName = enemyUnits[1].name
        e2Name = enemyUnits[0].name

    call FIGHT_05_MW_SETWISPS(limitValue(numWisp-1,0,5)) from _call_FIGHT_05_MW_SETWISPS_4
