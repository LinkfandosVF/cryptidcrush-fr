#Robyn's Moves -----------------------------------------------------------------
label FIGHT_01B_MM_ROBYN_0: ##Robyn's Bash. -- NEEDS TEXT
...
label FIGHT_01B_MM_ROBYN_1: ##Robyn's Cheer.
...
label FIGHT_01B_MM_ROBYN_2: ##Robyn's Focus. -- NEEDS TEXT
    $pName = playerUnits[0].name
    if isRollSuccess:
...
label FIGHT_01B_MM_ROBYN_3: ##Robyn's Heart Out.
    python:
        pName = playerUnits[0].name
        eName = enemyUnits[0].name
...
#Taro's Moves ------------------------------------------------------------------
label FIGHT_01B_MM_TARO_0: ##Taro's Pounce -- NEEDS TEXT
    python:
...
label FIGHT_01B_MM_TARO_1: ##Taro's Tuna Defender
    $pName = playerUnits[3].name
...
label FIGHT_01B_MM_TARO_2: ##Taro's Jeer
    #All Taro's Moves except tuna defender set the defending state off
    $eName = enemyUnits[0].name
...
label FIGHT_01B_MM_TARO_3: ##Taro's HexFlare -- NEEDS TEXT
    python:
        pName = playerUnits[3].name
        eName = enemyUnits[0].name
...
#Jamie's Moves -----------------------------------------------------------------
label FIGHT_01B_MM_JAMIE_0: ##Jamie's Spirit Blaze. -- NEEDS TEXT
    python:
...
label FIGHT_01B_MM_JAMIE_1: ##Jamie's Skull Cracker. -- NEEDS TEXT
    #Damage!!!
    python:
        pName = playerUnits[2].name
        eName = enemyUnits[0].name
...
label FIGHT_01B_MM_JAMIE_3: ##Jamie's Iron Tether. -- NEEDS TEXT
    #Sets Madhouse emote
    $MMD_State["hurt"] = True
    $MMD_State["glitch"] = 1
    $eName = "ERROR"
...
label FIGHT_01B_MM_JAMIE_2: ##Jamie's Healing Wave.
    $pName = playerUnits[2].name
...
label FIGHT_01B_MM_ATLAS_1: ##Atlas' Kinesis. -- NEEDS TEXT
    python:
...
label FIGHT_01B_MM_ATLAS_2: ##Atlas' Pump Up. -- NEEDS TEXT
    $pName = playerUnits[1].name
    $xName = playerUnits[2].name
    ...
    if isRollSuccess:
        ...
        python:
            ...
            playerUnits[2].modifyStatMod("brawn",3,-4,4)
...
label FIGHT_01B_MM_ATLAS_3: ##Atlas' Cryptic Collection.
    python:
        #Randomize Line
...
label FIGHT_01B_MM_MM_ATTACK:
    ...
    if xAtk == 0: #Phantom Slash
        call FIGHT_01B_MM_MM_ATTACK_1 from _call_FIGHT_01B_MM_MM_ATTACK_1

    elif xAtk == 1: #Noxious Blast
        call FIGHT_01B_MM_MM_ATTACK_2 from _call_FIGHT_01B_MM_MM_ATTACK_2

    elif xAtk == 2: #Psycho Toss Needs text
        call FIGHT_01B_MM_MM_ATTACK_3 from _call_FIGHT_01B_MM_MM_ATTACK_3

    else: #Psycho Spin Needs Text
        $eName = enemyUnits[0].name
        Narrator "[eName] cackles!"
        $enemyUnits[0].modifyStatMod("diff",1,-4,4)
        # call FIGHT_01B_MM_MM_ATTACK_4 from _call_FIGHT_01B_MM_MM_ATTACK_4
...
label FIGHT_01B_MM_MM_ATTACK_1:
    ...
    #Dice Roll
    call dice_roll(playerUnits[eTarget].cStats("hustle"), enemyUnits[0].cDifficulty("hustle"), "Phantom Slash") from _call_dice_roll_47
...
label FIGHT_01B_MM_MM_ATTACK_2:
    $HighlightPlayerUnitBars([0,1,2,3])
    ...
    if Taro_Stats.taroTuna: #Taro protects everyone
        ...
        #Roll
        call dice_roll(playerUnits[3].cStats("guts"), enemyUnits[0].cDifficulty("occult"), "Noxious Blast") from _call_dice_roll_48
        ...
        #Roll
        call dice_roll(xGuts, enemyUnits[0].cDifficulty("occult"), "Noxious Blast") from _call_dice_roll_49
...
label FIGHT_01B_MM_MM_ATTACK_3:
    python: #Calculate Damage
        xDmg = -(renpy.random.randint(6,10))
        atlasHelp = False

        #This move can't target Atls
        if eTarget == 1:
            eTarget = renpy.random.choice([0,2,3])
            eTargetName = playerUnits[eTarget].name
    ...
    if Taro_Stats.taroTuna: #Taro defender Needs Text
        ...
        $eTarget = 3
        $eTargetName = playerUnits[eTarget].name
        call dice_roll(playerUnits[3].cStats("brawn"), enemyUnits[0].cDifficulty("occult"), "Psycho Toss") from _call_dice_roll_50
        ...
        $Atlas_Stats.iconState = 3
        call dice_roll(playerUnits[1].cStats("occult"), enemyUnits[0].cDifficulty("occult"), "Psycho Toss") from _call_dice_roll_51
        show MMFight_Chair at MMFight_LeftProp_Exit
        show MMFight_Speaker at MMFight_RightProp_Exit
        if isRollSuccess:
            #Atlas Recoil
        ...
        #Roll
        call dice_roll(playerUnits[eTarget].cStats("guts"), enemyUnits[0].cDifficulty("occult"), "Psycho Toss") from _call_dice_roll_52
        show MMFight_Chair at MMFight_LeftProp_Exit
        show MMFight_Speaker at MMFight_RightProp_Exit
        if isRollSuccess:
...
label FIGHT_01B_MM_MM_ATTACK_4:
    $HighlightPlayerUnitBars([eTarget])
    Narrator "[eName] spins [eTargetName] around with psychic power!"

    call dice_roll(playerUnits[eTarget].cStats("guts"), enemyUnits[0].cDifficulty("occult"), "Psycho Spin") from _call_dice_roll_53
    ...
    if isRollSuccess:
        ...
        $playerUnits[2].modifyStatMod("guts",-1,-4,4)
        $playerUnits[2].modifyStatMod("brains",-1,-4,4)
        $playerUnits[2].modifyStatMod("hustle",-1,-4,4)
    return
